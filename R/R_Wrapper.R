...
AUDC <- function (X0,n,s,q,init="rand",initX=matrix(0),crit="MD2",maxiter=100000,hits_ratio = 0.1,vis=FALSE)
{
  ...
  else if(is.numeric(n)&&is.numeric(s)&&is.numeric(q) == FALSE){stop("Wrong types of n,s,q. End of program...")}
                  ...
                  hits_ratio = c(0.3,0.2,0.15,0.1,0.05,0.01),vis=FALSE)
{
  ...
  return_list = list()
  return_list$Aug_Matrix = D1
  return_list$Objective_Matrix = D_global
  return_list$obj = Dis_glob
  return_list$obj_list = Dis_list
  return_list$time_consumed = (proc.time() - start_time)[3]
