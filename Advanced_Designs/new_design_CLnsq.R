...
#set work directory to CL2_nq directory. Then create Improvement and Discrepancy directory if not existed.
PATH_TO_FILE = "./"
...
for(file in tmp_CL2_nq){
  tryCatch({
    ...
    if(length(cur_design)==0){total_CL2_files = total_CL2_files-1}
      ...
      for(i in 1:2){
        impro_list = UDC(init = "input",initX = best_design,crit = "CL2")
        impro_value = impro_list$obj
        
        impro_list2 = UDC(n=run,s=factor,q=level,init="rand",crit="CL2")
      ...
      if(best_value < cur_value - 1e-14){
        number_improved = number_improved+1
        ### save new design in Improvement directory:
        write.csv(best_design,file = paste("./Improvement/new_",file))
        tmpl = list()
