...
#set work directory to MD2_nn directory. Then create Improvement and Discrepancy directory if not existed.
PATH_TO_FILE = "./"
...
tmp_MD2_nn = list.files(path = PATH_TO_FILE,pattern="*.csv")
total_MD2_files = length(tmp_MD2_nn)
number_improved = 0
process = 1
ptm <- proc.time()
for(file in tmp_MD2_nn){
  tryCatch({
    ...
    if(length(cur_design)==0){total_MD2_files = total_MD2_files-1}
      ...
      for(i in 1:3){
        ...
        impro_list  =LP(X0 = impro_list$UniDOE_Matrix, crit = "MD2")
        impro_value = impro_list$obj
        
        impro_list2 = UDC(n=run,s=factor,q=level,init="rand",crit="MD2")
        impro_list2  =LP(X0 = impro_list2$UniDOE_Matrix, crit = "MD2")
