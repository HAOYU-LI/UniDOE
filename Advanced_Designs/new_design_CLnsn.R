...
#set work directory to CL2_nn directory. Then create Improvement and Discrepancy directory if not existed.
PATH_TO_FILE = "./"
...
tmp_CL2_nn = list.files(path = PATH_TO_FILE,pattern="*.csv")
total_CL2_files = length(tmp_CL2_nn)
number_improved = 0
process = 1
ptm <- proc.time()

for(file in tmp_CL2_nn){
  tryCatch({
